...
for i = 1:numAttempts
    ...
    % calculate the means for pts and modelPts
    ptsMean = repmat( mean(pts', 2), 1, m1 );
    modelPtsMean = repmat( mean(modelPts', 2), 1, m2 );
    
    % calculate the centroids
    ptsCentroid = pts' - ptsMean;
    modelPtsCentroid = modelPts' - modelPtsMean;
    
    % calculate the translation between the centroids
    translation = (ptsCentroid(:, 1) - modelPtsCentroid(:, 1))';
    
    % generate a vector with elements between -randRange and +randRange
    randRange = 0.95;
    randomOffset = [0 0 0] + (-randRange + (randRange+randRange)*rand(1));
    
    % add the randomOffset vector to the final translation
    initTrans = translation + randomOffset;
